...
class MixedDataModel:
    ...
    def __init__(self, config):
        self.config = config
        # Create folders to save the models and plots if they don't exist
        self.model_path = self.config.surrogate.train

    def normalize_data(self, samples, normalized_names_values=LARGEST_WEIGHT, normalized_membership_values=NUM_CLUSTERS):
        normalized_samples = {}
    ...
    def get_model(self, load_model=None):
        model = NetworkModel(num_embeddings=NUM_EMBEDDINGS,
    ...
    def train_model(self, load_model, train_loader, test_loader, model_path=None):
        # print(
        #     f"Input size: {input_size}, hidden size: {hidden_size}, output size: {output_size}")

        model, criterion, optimizer = self.get_model(load_model=load_model)
