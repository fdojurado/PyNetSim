...
class ClassificationModel(nn.Module):
    # This model is used for classification
...
def load_samples():
    samples = {}
    for file in os.listdir("data_classification"):
        # Ignore .DS_Store
        if file == ".DS_Store":
            continue
        # Load the data
        with open(os.path.join("data_classification", file), "r") as f:
            data = json.load(f)
        # Get the name only which is after LEACH-CE-E_ and before .json
...
def main(args):
    ...
    if args.data is None:
        samples = process_data(normalized_names_values=6,
    ...
    # Lets one hot encode the y and print the shape
    y = np.array(y)
    encoder = OneHotEncoder(sparse_output=False)
    y = encoder.fit_transform(y)

    # Lets convert the x to numpy
    x = np.array(x)
    ...
    model = ClassificationModel(
        train_dataset.x.shape[1], HIDDEN_SIZE, train_dataset.y.shape[1])
        ...
        with torch.no_grad():
            ...
            if avg_loss < best:
                ...
                torch.save(model.state_dict(), os.path.join(
                    MODELS_PATH, "classification_model.pt"))
            else:
